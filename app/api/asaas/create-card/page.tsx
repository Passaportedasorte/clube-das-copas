...
export async function POST(req: Request) {
  try {
    ...
    const couponCodeBody = String(body.couponCode || "").trim().toUpperCase();
    ...
    const codigoCupom =
      couponCodeBody ||
      String(profile?.referred_by || "").trim().toUpperCase();
    ...
    if (codigoCupom) {
      ...
      if (cupomDentroDoLimite) {
        ...
        desconto = Number(((valorOriginal * percentualDesconto) / 100).toFixed(2));
    ...
    if (!paymentResponse.ok) {
      return NextResponse.json(
        { error: "Erro ao criar cobrança no cartão", details: payment },
        { status: 400 }
      );
    }
    ...
    if (subscriptionError) {
      return NextResponse.json(
        {
          error: "Cobrança criada, mas erro ao salvar assinatura.",
    ...
    return NextResponse.json(
      {
        error: "Erro ao gerar cobrança no cartão.",
